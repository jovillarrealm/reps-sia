...
fn read_and_extract_data(pdf_contents: &str) -> Result<HashMap<String, Vec<Solicitud>>, String> {
    ...
    let solicitud_regex2 = Regex::new(r"(?s)nombre del estudiante\s*(.+?)\s*identificación\s*(\d+\s*\d*)\s*plan de estudios\s*(.+?)\s*número y fecha de la solicitud\s*([^ ]+)\s*\d*\s*(\d{2}/\d{12}/\d{4}|\d{2}/\d{2}/\d{2})\s*")
    .map_err(|e| format!("Error compiling regex: {}", e))?;
